...
async def call_claude(system_prompt: str, user_text: str, api_key: str, effort: str = "") -> str:
    cmd = [
        "claude", "-p",
        "--output-format", "text",
        "--tools", "",                                       
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"claude CLI exit {proc.returncode}: {(err or out)[:300]}")
                                       
    if out.startswith("API Error") or "forbiddenmodel" in out or "need " in out:
...
async def evaluate_case(case, system_prompt, semaphore, api_key, traces_dir, index, total, effort=""):
    async with semaphore:
        ...
        for retry in range(max_retries + 1):
            try:
                ...
                response_text = await call_claude(system_prompt, user_text, api_key, effort=effort)
                ...
                last_error = e
                es = str(e)
                if "429" in es or "Throttling" in es or "rate" in es.lower() or "overloaded" in es.lower():
                    await asyncio.sleep(10 * (retry + 1))
                    continue
                print(f"  [{index}/{total}] {task_id}:  error - {e}", flush=True)
                break
...
async def run_evaluation(args):
    print("=" * 70)
    print("LiveMathematicianBench theorem multiple-choice solving - evaluation (Claude) ")
    print("=" * 70)

    api_key = os.environ.get(MODEL_CONFIG["api_key_env"], "")
    print(f"\n model: claude / {MODEL_CONFIG['model_name']}")
    print(f"API: Claude Code CLI (claude -p)")
    ...
    output_base = Path(args.output_base) if args.output_base else (
        Path(__file__).parent.parent / "evolved" / "livemath-solver-claude")
    ...
    tasks = [
        evaluate_case(c, system_prompt, semaphore, api_key, traces_dir, i + 1, len(test_cases), effort=effort)
    ...
    summary = {
        "timestamp": datetime.now().isoformat(),
        "model": f"claude/{MODEL_CONFIG['model_name']}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LiveMath theorem multiple-choice solving - batchevaluation (Claude) ")
    ...
    parser.add_argument("--max-concurrent", "-c", type=int, default=3, dest="max_concurrent",
                        help="concurrencytask count (default 3, claude rate limitstrict , suggestionlowconcurrency) ")
    parser.add_argument("--output-base", "-o", default="", dest="output_base",
                        help="resultsoutputdirectory (default evolved/livemath-solver-claude) ")
    parser.add_argument("--model", "-m", default="", help="coveragemodelname (default claude-opus-4-7) ")
    parser.add_argument("--effort", default="", help="claude -p effort level (low/medium/high)")
