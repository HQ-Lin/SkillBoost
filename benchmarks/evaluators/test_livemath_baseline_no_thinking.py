...
async def run_evaluation(args):
    ...
    print("=" * 70)
    print("LiveMathematicianBench theorem multiple-choice solving - no  Skill Baseline (offthinking mode) ")
    print("=" * 70)

    api_key = openai_compatible_api_key(MODEL_CONFIG["api_key_env"])
    print(f"\n model: openai_compatible / {MODEL_CONFIG['model_name']}")
    print(f"thinking mode: off (enable_thinking=False)")
    ...
    run_dir = output_base / f"baseline_no_thinking_run_{timestamp}"
    ...
    summary = {
        ...
        "skill_path": "NONE (Baseline without Thinking)",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LiveMath Baseline (no  Skill + offthinking mode) - batchevaluation (DeepSeek v4 Pro) ")
