...
async def call_llm(messages: list, config: dict) -> str:
    if config["model_provider"] == "claude_cli":
        return await call_claude_cli(messages, config["model_name"])
    ...
    try:
        for cn in _generate_cell_names(cell_range):
            ...
            if not cn or len(cn) < 2:
                return False, f"invalid cell reference: {cn!r}"
            cg = ws_gt[cn]
            cp = ws_proc[cn]
                                                                           
            if isinstance(cg, tuple):
                if cg and isinstance(cg[0], tuple) and cg[0]:
                    cg = cg[0][0]                              
                elif cg and hasattr(cg[0], 'value'):
                    cg = cg[0]                         
                else:
                    cg = None
            if isinstance(cp, tuple):
                if cp and isinstance(cp[0], tuple) and cp[0]:
                    cp = cp[0][0]
                elif cp and hasattr(cp[0], 'value'):
                    cp = cp[0]
                else:
                    cp = None
            if cg is None or cp is None:
                return False, f"cell access failed: {cn}"
    ...
    for turn in range(max_turns):
        ...
        for retry in range(3):
            try:
                ...
                raw = await call_llm(messages, model_config)
                break
            except Exception as e:                
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", "-m", default="openai_compatible",
                        choices=["openai_compatible", "claude", "claude-cli", "qwen37max"],
                        help="modelchoose: openai_compatible(default), claude(HTTP), claude-cli(CLI pipe), qwen37max(qwen3.7-max)")
