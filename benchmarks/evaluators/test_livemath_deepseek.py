...
async def call_llm_api(messages: list, api_key: str, client) -> str:
    payload = {
...
async def evaluate_case_with_timeout(case, system_prompt, semaphore, api_key, traces_dir, index, total, client, timeout=1800.0):
...
async def run_evaluation(args):
    ...
    print("=" * 70)
    print("LiveMathematicianBench theorem multiple-choice solving - evaluation (DeepSeek v4 Pro) ")
    print("=" * 70)
    ...
    async with httpx.AsyncClient(timeout=600.0, limits=limits) as client:
        tasks = [
            evaluate_case_with_timeout(c, system_prompt, semaphore, api_key, traces_dir,
                          i + 1, len(test_cases), client, timeout=1800.0)              
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LiveMath theorem multiple-choice solving - batchevaluation (DeepSeek v4 Pro) ")
    ...
    parser.add_argument("--model", "-m", default="", dest="model",
                        help="coveragemodelname (default deepseek-v4-pro) ")
