...
def create_mutation_brief_list():
    ...
    modules = [
        ('1', 'Metadata Header', 
         ['Version, model, dataset, baseline metrics']),
        
        ('2', 'Baseline Performance Summary',
         ['Overall success rates (train vs. test)',
          'Per-category performance breakdown']),
        
        ('3', 'Failure Mode Cluster Analysis',
         ['Grouped failure patterns with root causes',
          'Representative case examples']),
        
        ('4', 'Mutation Strategy (Repair Actions)',
         ['Concrete modification proposals',
          'Exact rule text to be added/modified']),
        
        ('5', 'Mutation Action Mapping Table',
         ['Traceability matrix:',
          r'failure cluster $\rightarrow$ root cause $\rightarrow$ repair action $\rightarrow$ SKILL.md location']),
        
        ('6', 'Anti-Regression Guardrails',
         ['Must-maintain strengths',
          'Success thresholds and rejection criteria']),
        
        ('7', 'Back-Testing Results (Post-evolution, optional)',
         ['Targeted re-test on previously failed cases',
    ...
    for i, (num, title, descriptions) in enumerate(modules):
        ...
        line_height = 0.20                                    
        num_lines = len(descriptions)
        desc_block_height = (num_lines - 1) * line_height
    ...
    plt.tight_layout()
    plt.savefig('/path/to/project/analysis/figure_mutation_brief_modules.pdf',
                dpi=300, bbox_inches='tight', facecolor=COLORS['bg'])
    plt.close()
    
    print("✓ Generated: figure_mutation_brief_modules.pdf")
